...
static uint32_t inc_idx(uint32_t i) { return (i + 1u) % EVM_EVENT_QUEUE_SIZE; }
...
bool event_manager_enqueue(const ParsedEvent* event) {
  ...
  if (!is_valid_event_type(event->type)) {
    g_q.dropped++;
    EVM_CRIT_EXIT();
    logger_log(LOG_ERROR, millis(), 0xEE01);
    return false;
  }
...
bool event_manager_enqueue_timer(EventType type, uint32_t timestamp_ms,
                                 uint32_t data) {
  if (!is_valid_event_type(type)) {
    EVM_CRIT_ENTER();
    g_q.dropped++;
    EVM_CRIT_EXIT();
    logger_log(LOG_ERROR, millis(), 0xEE06);
    return false;
  }
...
bool event_manager_dispatch_one(void) {
  ...
  if (!is_valid_event_type(e.type)) {
    logger_log(LOG_ERROR, millis(), 0xEE05);
    return true;
  }
...
uint32_t event_manager_dispatch_all(void) {
  ...
  while (event_manager_dispatch_one()) {
    n++;
    // Hard-stop: prevent infinite loops from handlers that re-enqueue.
...
bool event_manager_is_drain_pending(void) { return g_drain_pending; }

bool event_manager_is_queue_full(void) {
  return g_q.count >= EVM_EVENT_QUEUE_SIZE;
}

uint32_t event_manager_get_queue_depth(void) { return g_q.count; }

uint32_t event_manager_get_total_enqueued(void) { return g_q.total_enqueued; }

uint32_t event_manager_get_dropped_count(void) { return g_q.dropped; }