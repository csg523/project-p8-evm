...
void power_monitor_init(void) {
    ...
    if      (rcause & RCAUSE_WDT)                 { _cause = RESET_WATCHDOG;   _recovery = true;  }
    else if (rcause & (RCAUSE_BOD33 | RCAUSE_BOD12)) { _cause = RESET_POWER_LOSS; _recovery = true;  }
    else if (rcause & RCAUSE_SYST)                { _cause = RESET_SOFTWARE;   _recovery = true;  }
    else if (rcause & RCAUSE_POR)                 { _cause = RESET_COLD_BOOT;  _recovery = false; }
    else                                          { _cause = RESET_UNKNOWN;    _recovery = false; }
