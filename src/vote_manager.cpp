...
void vote_manager_init(void) {
    ...
    for (uint32_t i = 0; i < n; i++) {
        VoteRecord r;
        if (storage_read_record(i, &r) == EVM_OK && r.candidate_id < EVM_MAX_CANDIDATES) {
...
EvmResult vote_manager_process(uint32_t vote_id, uint8_t candidate_id, uint32_t timestamp_ms) {
    ...
    if (r != EVM_OK) {
        logger_log(LOG_ERROR, timestamp_ms, (uint32_t)r);
        return r;
    }
    ...
    _vm.votes_processed++;
    _tally[candidate_id]++;
    logger_log(LOG_VOTE_ACCEPTED, timestamp_ms, vote_id);
    return EVM_OK;
}
